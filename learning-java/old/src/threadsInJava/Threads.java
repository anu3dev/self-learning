/*
 * thread is a execution of sequence of line.
 ...
 * 1. by extending threads
 * 2. by implementing runnable interface i.e. Runnable
 * 
 * thread object will be created using new keyword and Thread class, and thread will be only excuted through run method
 * thread schedular is boss of all threads, whether it is default thread or user created thread
 * once thread will be created then it will be handed over to thread schedular using start method
 * we have no control on thread schedular, JVM will manage it, thread schedular will make sure, CPU time should not be wasted
 */
 ...
 * thread will only be excuted through run method automatically
 */
 ...
 * only creation is not sufficient, we have to start the thread so it can be passed to thread schedular
 */
class ThreadsEx2 {
	public ThreadsEx2() {
...
/*
 * example - thread created - by extending threads class
 * thread will only be excuted through run method automatically
 */
