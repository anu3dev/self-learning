/*
 * exception comes only at runtime, not compile time
 * we can write custom exception handler using try and catch block to handle the smooth termination if 
 * unwanted things happen, else JVM will call default exception handler and abrupt termination will happen.
 * 
 * if excepion is generated:
 * 1. handle exception by try and catch
 * 2. duck the exception by throws keyword
 * 3. rethrowing the exception by throw, throws, try, catch , finally etc.
 * 
 * in java two category of exception
 * 1. identify and anticipate possibility of exception called as unchecked exception where copiler will not check
 * 2. compiler will produce warning like at this line, exception might occur 
 * for example example ExceptionalHandlingEx6 called checked exception
 * 
 * checked exception will also occour at runtime only, just only compiler is producing warning
 */
 ...
 * for example: it should be within java boundary only, it should not impact the whole CPU
...
class ExceptionalHandlingEx3 {
	...
	ExceptionalHandlingEx3(){
		...
		try {
			...
			/*
			 * pre-defined exception handler for airthemic operations
			 */
