### into

####
<details>
<summary> 🟩 about me </summary>

- My name is Anurag and I am working as Tech lead with Wipro, I have joined Wipro on June 2021 and since then I am working for US bank, in March 2023, I have travelled to Minneapolis and since then I am leading UI team for onboarding project with US bank.

- I have worked in multiple domains like banking, healthcare, e-commerce etc mainly with JavaScript based frameworks like React, TypeScript etc.

- I have also completed 3-4 hands on project on Java spring boot, goal is to support a team end to end, also I am ready to take responsiblity like 70:30 or 80:20, as of now heavier in UI side and bit lighter in backend side.

- Onboarding is a project which let user completes tasks like activate your card, enroll to Zelle etc. for all kind of peoducts like deposit, cards and lending inclucing 13000+ partners.

- I lead onboarding team to multiple org level migrations like state farm, enward jones, union bank, peru etc.

- my role is to handle merge request, helping product owner in translating business requirements to technical stories, resolving technical queries. Also leading monthly prod deployment and log monitoring.

</details>

